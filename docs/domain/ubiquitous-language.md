...
- **Personal Importance** — a bounded, ranking-specific behavioral signal learned from the user's own recorded Attention Dispositions (acted on vs. dismissed) toward a source-neutral **Originator** — the immutable `{namespace, id}` identifying who a piece of work is from ([ADR-0014](../adr/0014-personal-importance-from-dispositions.md), [#65](./README.md)). It answers one narrow question at ranking time: *does the user tend to act on, or dismiss, work from this originator?* Deliberately **not** a User Preference (it is inferred, not stated) and **not** a general belief about the person — it never leaves the ranking context, stays neutral without enough decisive history, and is fully rebuildable from the Event log like every other projection ([ADR-0009](../adr/0009-storage-strategy.md)).
...
> **Disambiguation — Personal Importance vs User Preference.**
> A **User Preference** is *stated*: the user told Orion directly, it is authoritative, and Rules honor it unconditionally. **Personal Importance** is *inferred*: Orion notices a behavioral pattern (acting on Dana's messages, dismissing a particular bot's) and lets that pattern nudge ranking, never a Rule. The user never "sets" a Personal Importance value the way they set a Preference, and it always stays a modest, bounded input — it can shift a Work Item's position, never suppress or force one.
