...
- **Actions are scoped to a revision.** Each current action records the exact occurrence ids the user saw (`basisEventIds`). An item resurfaces when a new revision appears (a basis id the action never covered) and stays quiet when only older facts trickle in.
