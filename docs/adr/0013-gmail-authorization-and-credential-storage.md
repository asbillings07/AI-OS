...
- **Callback validation.** On callback we require a refresh token, verify `gmail.readonly` was actually granted, and verify the authorized account matches `ORION_GMAIL_ACCOUNT` (via the Gmail profile). Any failure best-effort revokes the just-issued token and leaves any existing credential unchanged.
...
- **Refresh and rotation.** `getAccessToken()` refreshes automatically. If Google rotates the refresh token, the new one is persisted via the library's `tokens` event.
...
- **Follow-ups / new constraints:** Pagination, ret/backoff, and rate-limit handling for `LiveGmailSource` (this ADR fetches one page with a timeout); caching/versioning of advisory AI summaries before enabling a live provider; a real auth story if Orion is ever exposed beyond localhost.
