...
- **Ranking stays Context-independent and explainable.** `prioritize()` receives a precomputed numeric importance contribution, never `ContextState`; the "why" is evidence-specific and source-neutral ("You've acted on more work from {name} than you've dismissed."). Importance provenance is exposed separately from the presentation revision (`attentionBasisEventIds`). *(The ranking integration itself lands in the second PR of #65; this ADR fixes the model.)*
